...
fn parse_head_tail_tup(
    ...
    let tail = tail.ok_or(())?;
    Ok((convert.subtract(start),
        Sym(Symbol::HeadTailTup(Box::new(Sym(Var(head))), Box::new(Sym(Var(tail)))))))
}
...
fn parse_knowledge(
    ...
    loop {
        ...
        } else if let Ok((range, _)) = convert.meta_bool("def") {
            convert.update(range);
            op = Some(KnowledgeOp::Def);
        } else {
            let range = convert.ignore();
            convert.update(range);
            ignored.push(range);
    ...
    let knowledge = match op {
        ...
        KnowledgeOp::Def => if let Sym(x) = left {Knowledge::Def(x, right)} else {return Err(())},
    };
