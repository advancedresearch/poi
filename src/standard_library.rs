...
pub fn std() -> Vec<Knowledge> {
    vec![
        ...
        // `(\a * x) + x => (a + 1) * x`
        Red(app2(Add, app2(Mul, ret_var("a"), "x"), "x"), app2(Mul, app2(Add, "a", 1.0), "x")),
        // `(\a * x) - x => (a - 1) * x`
        Red(app2(Sub, app2(Mul, ret_var("a"), "x"), "x"), app2(Mul, app2(Sub, "a", 1.0), "x")),
        ...
        Red(app(constr(Not, app(Rty, VecType)), head_tail_list("x", "y")),
            app2(Concat, List(vec![app(Not, "x")]), app(constr(Not, app(Rty, VecType)), "y"))),
        // `neg{(: vec)}([x]) => [neg(x)]`
        Red(app(constr(Neg, app(Rty, VecType)), List(vec!["x".into()])), List(vec![app(Neg, "x")])),
        // `neg{(: vec)}([x, y..]) => [neg(x)] ++ neg{(: vec)}(y)`
        Red(app(constr(Neg, app(Rty, VecType)), head_tail_list("x", "y")),
            app2(Concat, List(vec![app(Neg, "x")]), app(constr(Neg, app(Rty, VecType)), "y"))),
        ...
        Red(app(constr(Transpose, app(Rty, VecType)), "x"), unop_ret_var("x", Transpose)),
        ...
        // `integ(!\x)(c)(\k) => (c + k * x)`
        Red(app3(Integ, not_ret_var("x"), "c", ret_var("k")),
            app2(Add, "c", app2(Mul, "k", "x"))),
        // `integ(!\x)(c)(\k * y) => (k * integ(x)(c / k)(y))`
        Red(app3(Integ, not_ret_var("x"), "c", app2(Mul, ret_var("k"), "y")),
            app2(Mul, "k", app3(Integ, "x", app2(Div, "c", "k"), "y"))),
        ...
        // `integ(!\x)(c)(sin(x)) => (c + neg(cos(x)))`
        Red(app3(Integ, not_ret_var("x"), "c", app(Sin, "x")),
            app2(Add, "c", app(Neg, app(Cos, "x")))),
        ...
        // `integ(x, c, d(x, y) * exp(y)) => (c + exp(y))`
        Red(app3(Integ, not_ret_var("x"), "c", app2(Mul, app2(D, "x", "y"), app(Exp, "y"))),
            app2(Add, "c", app(Exp, "y"))),
        ...
        // `(x * x) => x^2`
        Red(app2(Mul, "x", "x"), app2(Pow, "x", 2.0)),
        ...
        // `item(0) . dim => len`
        Red(comp(app(Item, 0.0), Dim), Len.into()),
        // `(f . fst){x}(a){_}(_) => f{x}(a)`
        ...
        // `(-(x * y)) => (-x * y)`
        Red(app(Neg, app2(Mul, "x", "y")), app2(Mul, app(Neg, "x"), "y")),
        // `add(!\a)(\b) => add(b)(a)`
        ...
        // `(a - b = c) <=> (a - (b - b) = c + b)`
        Eqv(app2(Eq, app2(Sub, "a", "b"), "c"),
            app2(Eq, app2(Sub, "a", app2(Sub, "b", "b")), app2(Sub, "c", "b"))),
        // `(a + b = c) <=> (b = c - a)`
        Eqv(app2(Eq, app2(Add, "a", "b"), "c"),
            app2(Eq, "b", app2(Sub, "c", "a"))),
        // `(a + b = c) <=> (a = c - b)`
        Eqv(app2(Eq, app2(Add, "a", "b"), "c"),
        ...
        Eqv(app2(Eq, app2(Add, "a", "b"), app2(Add, "c", "b")),
            app2(Eq, "a", "c")),
        // `mul(-1) <=> neg`
        Eqv(app(Mul, -1.0), Neg.into()),
        // `(a - b = c - b) <=> (a = c)`
        Eqv(app2(Eq, app2(Sub, "a", "b"), app2(Sub, "c", "b")),
            app2(Eq, "a", "c")),
        // `(a / b = c / b) <=> (a * (b / b) = c * (b / b))`
        Eqv(app2(Eq, app2(Div, "a", "b"), app2(Div, "c", "b")),
            app2(Eq, app2(Mul, "a", app2(Div, "b", "b")),
                     app2(Mul, "c", app2(Div, "b", "b")))),
        // `(a / c + b / c) <=> ((a + b) / c)`
        Eqv(app2(Add, app2(Div, "a", "c"), app2(Div, "b", "c")),
            app2(Div, app2(Add, "a", "b"), "c")),
        // `((a * b) / c) <=> (a * (b / c))`
        Eqv(app2(Div, app2(Mul, "a", "b"), "c"), app2(Mul, "a", app2(Div, "b", "c"))),
        // `(a * b - a) <=> (a * (b - 1))`
        Eqv(app2(Sub, app2(Mul, "a", "b"), "a"), app2(Mul, "a", app2(Sub, "b", 1.0))),
        // `(- y)(x) <=> (x - y)`
        ...
        // `ln(y ^ x) <=> x * ln(y)`
        Eqv(app(Ln, app2(Pow, "y", "x")), app2(Mul, "x", app(Ln, "y"))),
        // `log2(y ^ x) <=> x * log2(y)`
        Eqv(app(Log2, app2(Pow, "y", "x")), app2(Mul, "x", app(Log2, "y"))),
        // `log10(y ^ x) <=> x * log10(y)`
        Eqv(app(Log10, app2(Pow, "y", "x")), app2(Mul, "x", app(Log10, "y"))),
        // `ln(x * y) <=> ln(x) + ln(y)`
        Eqv(app(Ln, app2(Mul, "x", "y")), app2(Add, app(Ln, "x"), app(Ln, "y"))),
        // `log10(x * y) <=> log10(x) + log10(y)`
        Eqv(app(Log10, app2(Mul, "x", "y")), app2(Add, app(Log10, "x"), app(Log10, "y"))),
        // `log2(x * y) <=> log2(x) + log2(y)`
        Eqv(app(Log2, app2(Mul, "x", "y")), app2(Add, app(Log2, "x"), app(Log2, "y"))),
        // `exp(x + y) <=> exp(x) * exp(y)`
        Eqv(app(Exp, app2(Add, "x", "y")), app2(Mul, app(Exp, "x"), app(Exp, "y"))),
        // `exp(x * ln(y)) <=> y^x`
        Eqv(app(Exp, app2(Mul, "x", app(Ln, "y"))), app2(Pow, "y", "x")),

        // `(f . g) <=> f[inv(g) -> id]`
        Eqv(comp("f", "g"), path("f", (app(Inv, "g"), Id))),
        // `(f . (g0 . fst, g1 . snd)) <=> f[inv(g0) x inv(g1) -> id]`
        Eqv(comp("f", (comp("g0", Fst), comp("g1", Snd))),
            path("f", (app(Inv, "g0"), app(Inv, "g1"), Id))),
        // `(f . inv(g)) <=> f[g -> id]`
        Eqv(comp("f", app(Inv, "g")), path("f", ("g", Id))),
        // `(f . (inv(g0) . fst, inv(g1) . snd)) <=> f[g0 x g1 -> id]`
        Eqv(comp("f", (comp(app(Inv, "g0"), Fst), comp(app(Inv, "g1"), Snd))), path("f", ("g0", "g1", Id))),
        ...
        Eqv(app2(Eq, comp("g2", comp("f", "g1")), "h"),
            app2(Eq, "f", comp(app(Inv, "g2"), comp("h", app(Inv, "g1"))))),
        // `((f . g) = h) <=> ((inv(f) . f) . g)) = (inv(f) . h))`
        Eqv(app2(Eq, comp("f", "g"), "h"),
            app2(Eq, comp(comp(app(Inv, "f"), "f"), "g"), comp(app(Inv, "f"), "h"))),
        ...
        Eqv(app2(Eq, path("f", ("g0", "g1", "g2")), "h"),
            app2(Eq, "f", path("h", (app(Inv, "g0"), app(Inv, "g1"), app(Inv, "g2"))))),
